...
def loop():
    global sentAck, receivedAck, timePollAckSentTS, timePollReceivedTS, timePollSentTS, timePollAckReceivedTS, timeRangeReceivedTS, protocolFailed, data, expectedMsgId, timeRangeSentTS, tag_list, myAddress, nodeType,prev_anchor_init_over, prev_anch_flag
    ...
    if sentAck:
        ...
        if msgId == C.POLL_ACK:
            ...
            expectedMsgId[data[17]] = C.RANGE
            # tag.expectedmesg = C.RANGE_REPORT
            noteActivity()
        if msgId == C.RANGE_REPORT:
            expectedMsgId[data[17]] = C.POLL
            # tag.expectedmesg = C.POLL_ACK
    ...
    if receivedAck:
        ...
        prev_anchor_init_over = datatmp[20]
	
        # in_anchor_list = datatmp[21]
        ...
        if nodeType == typeOfSender:
            ...
            # print data
            noteActivity()
            # print "prev_anchor_init_over" , prev_anchor_init_over
            # if (sender == (myAddress - 1)) and (prev_anchor_init_over == 0) and (prev_anch_flag == 0) and (msgId == 0) : 
            #     # anchor_wait = 0
            #     prev_anch_flag = 1
            #     for i in tag_list.keys():
            #         print "AAAA"
            #         noteActivity()
            #         # print "tag_list[i].in_anchor ", tag_list[i].in_anchor
            #         # tag_list[i].in_anchor = 1
            #         return  transmitPollAck(tag_list[i].address,1) #,tag_list[i].in_anchor) #sec arg = 0
                
            return


        elif sender not in tag_list:
            ...
            if myAddress==1:
                transmitPollAck(sender,1)
                noteActivity()
                # tag_list[sender].in_anchor = 1
                # anchor_wait == 0 or
            print tag_list
        else:
            if receiver == 0xFF:
            ...
            #     resetInactive()
                ...
                if msgId == C.POLL:
                    ...
                    transmitPollAck(sender)
                    # tag_list[sender].in_anchor = 0 # initialization over
                    expectedMsgId[sender] = C.RANGE
                    
                    
                    noteActivity()
                if msgId == C.RANGE:
                    print "Got Range for sequence" , sequence
                    ...
                    if protocolFailed == False:
                        ...
                        noteActivity()
                        # expectedMsgId[sender] = C.POLL
                        # expectedmesg[nextSender] = C.POLL_ACK

                        # nextSender = nextTagwaitingFor(C.RANGE)

                        # if not nextSender == False :
                        #     transmitRangeAcknowledge(nextSender)
                        #     expectedMsgId[nextSender] = C.POLL
                        #     expectedmesg[nextSender] = C.POLL_ACK
